...
define([
    ...
    function initConf() {
        ...
        if (confName) {
            ...
            confParams.launchArguments = replaceInObject(confParams.launchArguments, new RegExp(rootReg), '${workspaceRoot}')
            return new Deferred().resolve();
        } else {
    ...
    function renderServerPage() {
        ...
        if (confParams.remote) {
            lib.$('#debug-server-input').value = confParams.debugServer || '';
            lib.$('#remote-root-input').value = confParams.remoteRoot || '';
        }
        handleRemoteCheckboxChanged();
        handleDebugServerChanged();
        remoteCheckbox.addEventListener('change', handleRemoteCheckboxChanged);
        lib.$('#debug-server-input').addEventListener('change', handleDebugServerChanged);
        ...
        require(['orion/editor/edit'], function(edit) {
            ...
            editor.getModel().addEventListener('Changed', function() {
                wizard.validate();
            });
            wizard.validate();
        });
        ...
        if (useRemote) {
            ...
            xhr('GET', new URL('debug/workspacePath', debugServerUrl.href).href, {
                ...
                if (!remoteCheckbox.checked) {
                    if (result.status === 200) {
                        lib.$('#remote-root-input').value = result.response.replace(/\/$/, '') + '/' + projName;
                        handleDebugServerChanged();
                    } else {
                        forceRemote = true;
                        remoteCheckbox.setAttribute('disabled', 'disabled');
    ...
    function handleDebugServerChanged() {
        ...
        if (adapterSelect.selectedIndex >= 0) {
            currentAdapter = adapterSelect.options[adapterSelect.selectedIndex];
        }
