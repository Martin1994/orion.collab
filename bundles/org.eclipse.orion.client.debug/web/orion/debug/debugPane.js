...
define([
    ...
    'orion/section'
], function(lib, mDebugSocket, mBreakpoint, mSection) {
    ...
    DebugPane.prototype._createDomNode = function() {
        ...
        buttons.appendChild(this._createDebugCommandButton('debugContinue', 'Continue', true));
        buttons.appendChild(this._createDebugCommandButton('debugPause', 'Pause', true));
        buttons.appendChild(this._createDebugCommandButton('debugStepOver', 'Step Over', true));
        buttons.appendChild(this._createDebugCommandButton('debugStepIn', 'Step In', true));
        buttons.appendChild(this._createDebugCommandButton('debugStepOut', 'Step Out', true));
        ...
        // Clear button
        var outputClearButton = this._createDebugCommandButton('debugClearOutput', 'Clear', false);
        ...
        // Add button
        var watchAddButton = this._createDebugCommandButton('debugAddWatch', 'Add', false);
        ...
        exceptionBreakpointAllLabel.innerText = 'All Exceptions';
        ...
        exceptionBreakpointUncaughtLabel.innerText = 'Uncaught Exceptions';
     ...
     * @return {Element}
     */
    DebugPane.prototype._createDebugCommandButton = function(className, displayText, disabled) {
    ...
    DebugPane.prototype._handleStatusEvent = function(e) {
        var i, buttons = lib.$$('.debugButtons button', this._domNode);
            ...
            breakpoints.forEach(function(breakpoint) {
                if (breakpoint instanceof mBreakpoint.LineBreakpoint || breakpoint instanceof mBreakpoint.LineConditionalBreakpoint) {
                    ...
                    nameElement.addEventListener('click', function(e) {
                        // TODO: just jump to that line without highlighting?
                        that._debugService.focusLine(breakpoint.location, breakpoint.line);
                    });
                    removeElement.addEventListener('click', function(e) {
                    ...
                    var exceptionElement = lib.$('.exceptionBreakpoint[extype="' + breakpoint.label + '"]', this._breakpointsElement);
                    if (exceptionElement) {
    ...
    DebugPane.prototype._evaluateScope = function(frameId) {
        ...
        lib.$$('.watch', this._watchListElement).forEach(function(watchElement) {
        ...
        if (frameId) {
            ...
            this._debugSocket.request('scopes', {
                ...
                if (!response.success) {
                    console.error('Failed to load scopes of stack fram #' + frameId + '.');
                    return;
                }
            ...
            moreElement.addEventListener('click', function(e) {
                ...
                if (subvarElement) {
                    ...
                    that._debugSocket.request('variables', {
                        ...
                        if (!response.success) {
                            console.error('Failed to get variable #' + reference + '.');
                            return;
                        }
        ...
        if (allThreadsContinued) {
            ...
            if (this._currentThread = threadId) {
                if (Object.keys(this._stoppedThreads).length > 0) {
                    this._currentThread = this._stoppedThreads[0];
                } else {
                    this._currentThread = undefined;
                    this._debugSocket.setStatus(mDebugSocket.StatusEvent.STATUS.RUNNING);
                }
        ...
        if (threadElement) {
            ...
            for (var i = 0; i < response.body.stackFrames.length; i++) {
                ...
                if (frame.source) {
                    ...
                    frameElement.addEventListener('click', (function(that, projectLocation, line, threadId, frameId) { return function(e) {
                        that._enterFrame(frameId, projectLocation, line, threadId); // Create a closure here
                    };})(this, projectLocation, frame.line, threadId, frame.id));
                    ...
                    if (!firstFrameFocused && (!frame.source.presentationHint || frame.source.presentationHint === 'emphasize')) {
                        ...
                        this._enterFrame(frame.id, projectLocation, frame.line, threadId);
                    }
                }
    ...
    DebugPane.prototype._getSourceProjectPath = function(source) {
        if (source.sourceReference) {
            return '/debug/file/' + this._debugSocket.getConnectionId() + '/' + source.sourceReference + '/' + source.path;
        } else {
     ...
     * @param {number} threadId
     */
    DebugPane.prototype._enterFrame = function(frameId, location, line, threadId) {
        this._currentThread = threadId;
    ...
    /**
     * Append new output element and scroll to the bottom
     * @private
