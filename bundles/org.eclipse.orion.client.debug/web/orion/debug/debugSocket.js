...
define([
    ...
    'orion/debug/debugHover'
], function(io, Deferred, i18nUtil, EventTarget, mBreakpoint, mDebugHover) {
     ...
     * 
     * Currently it is a singleton. However it is possible to make multiple debug sockets.
     * 
    ...
    var DebugSocket = function(serviceRegistry) {
        ...
        this._remoteRoot = '';
        this._remoteRootRegExp = null;
        this._localRoot = '';
        this._localRootRegExp = null;
        ...
        this._hoverService = new mDebugHover.DebugHover(serviceRegistry, this);
        serviceRegistry.registerService("orion.debug.debugSocket", this);
    };
    ...
    DebugSocket.prototype.launch = function(config) {
        ...
        this._remoteRootRegExp = new RegExp('^' + this._remoteRoot);
        this._localRoot = this._project.Location.replace(/\/$/, '');
        this._localRootRegExp = new RegExp('^' + this._localRoot);

        var serverUrl = new URL(config.debugServer);
    ...
    DebugSocket.prototype.absoluteToProjectPath = function(path) {
        return path.replace(this._remoteRootRegExp, this._localRoot);
    };
    ...
    DebugSocket.prototype.projectToAbsolutePath = function(path) {
        return path.replace(this._localRootRegExp, this._remoteRoot);
    };
            ...
            var setNext = function(response) {
                if (++i >= locations.length) {
                    that._debugService.getGlobalBreakpoints().then(function(breakpoints) {
                        that._setExceptionBreakpoints(breakpoints, function(response) {
                            callback();
                        });
                    });
                    return;
    ...
    DebugSocket.prototype._setBreakpointsForFile = function(location, breakpoints, callback) {
        ...
        if (location.startsWith('/debug')) {
            var reference = parseInt(location.split('/')[4], 10);
        ...
        this.request('setBreakpoints', {
            ...
            if (!response.success) {
                console.error('Failed to set breakpoint at ' + location + '.');
            } else {
                response.body.breakpoints.forEach(function(breakpoint) {
                    if (!breakpoint.verified) {
                        console.error(location + ':' + breakpoint.line + ' is not a valid breakpoint.');
                    }
                });
            }
            callback();
    ...
    DebugSocket.prototype._setExceptionBreakpoints = function(breakpoints, callback) {
        ...
        this.request('setExceptionBreakpoints', {
            ...
            if (!response.success) {
                console.error('Failed to set exception breakpoints.');
            }
            callback();
        });
    };
